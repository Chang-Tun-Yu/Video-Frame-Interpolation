...
def interpolate_frames(src, dst, flow0_1, flow1_0, n_frames):
    h, w, ch = src.shape
    x_src = np.arange(w)
    y_src = np.arange(h)
    y_coords, x_coords = np.mgrid[0:h, 0:w]
    src_cood = np.float32(np.dstack([x_coords, y_coords]))
    frames = []
    
    for i in range(1, n_frames):
        pixel_map0 = src_cood + (i/n_frames) * flow1_0
        pixel_map1 = src_cood + 1-(i/n_frames) * flow0_1
        inter_frame0 = cv.remap(src, pixel_map0, None, cv.INTER_LINEAR)
        inter_frame1 = cv.remap(src, pixel_map1, None, cv.INTER_LINEAR)
        frame0_1, occlution_mask0 = forward_warping(src, dst, i*0.125, flow0_1)
        frame1_0, occlution_mask1 = forward_warping(dst, src, 1 - i*0.125, flow1_0)
        # frame = splatting(inter_frame0, inter_frame1, occlution_mask0, occlution_mask1, i*0.125)
        # frame = (inter_frame0 + inter_frame1)/2.0
        frames.append(inter_frame0)
    return frames
...
def interp_frame(img0, img1, mode):
    if (mode == '1'):
        ...
        flow0_1 = optical_flow(img0, img1)        
        flow1_0 = optical_flow(img1, img0)
        # frames = interpolate_frames(img0, img1, flow0_1, flow1_0, 8)
        for i in range(1, 8):
